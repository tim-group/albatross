...
class IntervalSetSpec extends Specification {
  ...
  "An interval set" ^
    "orders its constituent intervals" ! {
      IntervalSet(open(10) to open(20), open(0) to open(5)).nonEmptyContinuousSubIntervals must_== List(open(0) to open(5), open(10) to open(20))
    } ^
    "coalesces its constituent intervals" ! {
      IntervalSet(open(0) to open(5), open(10) to open(15), open(3) to open(12)).nonEmptyContinuousSubIntervals must_== List(open(0) to open(15))
    } ^
  ...
  "The intersection of two interval sets" ^
    "contains the intersections of all sets in either interval set" ! {
      (IntervalSet(open(0) to open(10), open(20) to open(30)) intersect
       IntervalSet(open(5) to open(25), open(28) to open(35))).nonEmptyContinuousSubIntervals must_== List(open(5) to open(10), open(20) to open(25), open(28) to open(30))
    } ^ end
